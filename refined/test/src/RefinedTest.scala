  ...
  val tests = Tests {
    ...
    test("NIF unhappy path"):
      ...
      test("Invalid Number"):
        NIF("1234567AT") match
          case error: String =>
            assert(error == "Should have an exact length of 8 & For each element: (Should be a digit)")
      ...
      test("Invalid Control Letter"):
        NIF("12345678Ñ") match
          case error: String => assert(error == "Should match [TRWAGMYFPDXBNJZSQVHLCKE]{1}")
          case _: NIF        => assert(false)
    ...
    test("NIE unhappy path"):
      ...
      test("Invalid NIE Letter"):
        NIE("A1234567T") match
          case error: String => assert(error == "Should match [XYZ]{1}")
          case _: NIE        => assert(false)

      test("Invalid Number"):
        NIE("Y234567AT") match
          case error: String =>
            assert(error == "Should have an exact length of 7 & For each element: (Should be a digit)")
          case _: NIE => assert(false)
    ...
    test("ID unhappy path"):
      test("Invalid Input: empty"):
        validateID("         ") match
          case error: String => assert(error == "Should be alphanumeric & Should have an exact length of 9")
